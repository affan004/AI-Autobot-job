...
from utils.chrome_utils import chrome_browser_options, close_browser, launch_playwright_sync
...
class ConfigValidator:
    ...
    @staticmethod
    def validate_secrets(secrets_yaml_path: Optional[Path]) -> str:
        secrets = {}
        if secrets_yaml_path and secrets_yaml_path.exists():
            secrets = ConfigValidator.validate_yaml_file(secrets_yaml_path) or {}

        llm_api_key = secrets.get("llm_api_key")
        if not llm_api_key:
            llm_api_key = os.getenv("GOOGLE_AI_STUDIO_API_KEY") or os.getenv(
                "LLM_API_KEY"
            )

        if not llm_api_key:
            raise ConfigError(
                "Missing llm_api_key. Set it in data_folder/secrets.yaml or "
...
class FileManager:
    ...
    def validate_data_folder(app_data_folder: Path) -> tuple:
        ...
        secrets_path = app_data_folder / SECRETS_YAML
        return (
            secrets_path if secrets_path.exists() else None,
...
def init_browser() -> Any:
    if config.BROWSER_ENGINE == "playwright":
        try:
            logger.info(
                f"Initializing Playwright browser (headless={config.BROWSER_HEADLESS})"
            )
            return launch_playwright_sync(headless=config.BROWSER_HEADLESS)
        except Exception as e:
            logger.warning(
                f"Playwright initialization failed, falling back to Selenium: {str(e)}"
            )

    try:
        logger.info("Initializing Selenium browser")
